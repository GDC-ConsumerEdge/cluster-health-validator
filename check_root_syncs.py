...
class CheckRootSyncs:
    def is_healthy(self):
        ...
        resp = k8s.list_namespaced_custom_object(group="configsync.gke.io", version="v1beta1", plural="rootsyncs", namespace="config-management-system")
        ...
        for root_sync in resp.get("items"):
            ...
            reconciling_condition = [condition for condition in sync_conditions if condition.get("type") == "Reconciling"][0]
            if reconciling_condition.get("status") != "False":
                log.error(f'RootSync {root_sync.get("name")} is still reconciling')
                return failedCheck

            syncing_condition = [condition for condition in sync_conditions if condition.get("type") == "Syncing"][0]
            if syncing_condition.get("status") != "False" or syncing_condition.get("message") != "Sync Completed":
                log.error(f'RootSync {root_sync.get("metadata").get("name")} syncing not complete')
                return failedCheck
