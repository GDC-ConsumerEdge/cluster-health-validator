...
log = logging.getLogger("check.networkspeed")

default_image = "python:3-alpine"


class CheckNetworkSpeedParameters(BaseModel):
...
class CheckNetworkSpeed:
    ...
    This check can be configured to run only periodically by setting
    `minRunIntervalSeconds`. If set, the check will only execute if at least
    that many seconds have passed since its last run.
    """

    def __init__(self, parameters: dict) -> None:
        params = CheckNetworkSpeedParameters(**parameters)
    ...
    @with_interval_check
    def is_healthy(self):
        log.info("Running network speed test on all nodes...")
