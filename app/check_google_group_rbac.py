...
class CheckGoogleGroupRBAC:
    def is_healthy(self):
        ...
        resp = k8s.list_namespaced_custom_object(group="authentication.gke.io", version="v2alpha1", plural="clientconfigs", namespace="kube-public")

        try:
            clientconfig = resp.get("items")[0]

            if (clientconfig.get("metadata").get("name") != "default"):
                log.error(f'Did not find expected default.kube-public clientconfig object')
                return False
